...
class AccountGenerator:
    ...
    def load_proxies(self):
        try:
            ...
            print(f"{Fore.YELLOW}loaded {len(self.proxies)} proxies, testing... (this will take a while if you have a lot of proxies, but it depends about your network speed)")
            self.test_proxies()
    ...
    def send_verification(self, session, email):
        try:
            resp = session.post(
                "https://api.hyper3d.com/api/user/send_email_verification_code",
    ...
    def get_verification_code(self, session, email):
        for _ in range(10):
            try:
                ...
                messages = resp.json()
                if messages:
                    body = messages[0].get("body_text", "")
                    import re
                    match = re.search(r"Verification Code: \*(\d+)\*", body)
                    if match:
    ...
    def register(self, session, email, code):
        ...
        try:
            resp = session.post(
                "https://api.hyper3d.com/api/user/register",
                json={
                    "email": email,
            ...
            if resp.status_code == 201:
                with self.lock:
                    with open("hyper3d_accs.txt", "a") as f:
    ...
    def generate_account(self, thread_id):
        ...
        try:
            ...
            if not self.send_verification(session, email):
                with self.lock:
                    self.unsuccessful += 1
                self.remove_failed_proxy(proxy, queue)
                return True
    ...
    def start(self):
        print(f"\n{Fore.CYAN}{'=' * 50}")
        print(f"{Fore.CYAN}hyper3d.ai account generator")
